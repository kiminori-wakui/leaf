...
static LF_Value
eval_boolean_expression(LF_Boolean boolean_value)
{
    LF_Value   v;

    v.type = LF_BOOLEAN_VALUE;
...
static LF_Value
eval_string_expression(LF_Interpreter *inter, char *string_value)
{
    LF_Value   v;

    v.type = LF_STRING_VALUE;
    v.u.string_value = lf_literal_to_lf_string(inter, string_value);
...
eval_null_expression(void)
{
    ...
    v.type = LF_NULL_VALUE;

    return v;
}

static void
refer_if_string(LF_Value *v)
{
    if (v->type == LF_STRING_VALUE) {
        lf_refer_string(v->u.string_value);
    }
}

static void
release_if_string(LF_Value *v)
{
    if (v->type == LF_STRING_VALUE) {
        lf_release_string(v->u.string_value);
    }
}

static Variable *
search_global_variable_from_env(LF_Interpreter *inter,
                                LocalEnvironment *env, char *name)
{
    GlobalVariableRef *pos;

    if (env == NULL) {
        return lf_search_global_variable(inter, name);
    }
...
static LF_Value
eval_identifier_expression(LF_Interpreter *inter,
                           LocalEnvironment *env, Expression *expr)
{
    LF_Value   v;
    Variable    *vp;

    vp = lf_search_local_variable(env, expr->u.identifier);
    if (vp != NULL) {
        v = vp->value;
    } else {
        ...
        if (vp != NULL) {
            ...
            lf_runtime_error(expr->line_number, VARIABLE_NOT_FOUND_ERR,
...
static LF_Value eval_expression(LF_Interpreter *inter, LocalEnvironment *env,
                                 Expression *expr);
static LF_Value
eval_assign_expression(LF_Interpreter *inter, LocalEnvironment *env,
                       char *identifier, Expression *expression)
{
    ...
    left = lf_search_local_variable(env, identifier);
    if (left == NULL) {
    ...
    if (left != NULL) {
        ...
        if (env != NULL) {
            lf_add_local_variable(env, identifier, &v);
        } else {
            LF_add_global_variable(inter, identifier, &v);
        }
        refer_if_string(&v);
    }
...
static LF_Boolean
eval_binary_boolean(LF_Interpreter *inter, ExpressionType operator,
                    LF_Boolean left, LF_Boolean right, int line_number)
{
    LF_Boolean result;
    ...
    if (operator == EQ_EXPRESSION) {
        ...
        char *op_str = lf_get_operator_string(operator);
        lf_runtime_error(line_number, NOT_BOOLEAN_OPERATOR_ERR,
...
static void
eval_binary_int(LF_Interpreter *inter, ExpressionType operator,
                int left, int right,
                LF_Value *result, int line_number)
{
    if (dkc_is_math_operator(operator)) {
        result->type = LF_INT_VALUE;
    } else if (dkc_is_compare_operator(operator)) {
        result->type = LF_BOOLEAN_VALUE;
    } else {
...
static void
eval_binary_double(LF_Interpreter *inter, ExpressionType operator,
                   double left, double right,
                   LF_Value *result, int line_number)
{
    if (dkc_is_math_operator(operator)) {
        result->type = LF_DOUBLE_VALUE;
    } else if (dkc_is_compare_operator(operator)) {
        result->type = LF_BOOLEAN_VALUE;
    } else {
        DBG_panic(("operator..%d\n", operator));
...
eval_compare_string(ExpressionType operator,
                    LF_Value *left, LF_Value *right, int line_number)
{
    LF_Boolean result;
    int cmp;
    ...
    if (operator == EQ_EXPRESSION) {
        ...
        char *op_str = lf_get_operator_string(operator);
        lf_runtime_error(line_number, BAD_OPERATOR_FOR_STRING_ERR,
                          STRING_MESSAGE_ARGUMENT, "operator", op_str,
                          MESSAGE_ARGUMENT_END);
    }
    lf_release_string(left->u.string_value);
    lf_release_string(right->u.string_value);

    return result;
}

static LF_Boolean
eval_binary_null(LF_Interpreter *inter, ExpressionType operator,
                 LF_Value *left, LF_Value *right, int line_number)
{
    LF_Boolean result;

    if (operator == EQ_EXPRESSION) {
        result = left->type == LF_NULL_VALUE && right->type == LF_NULL_VALUE;
    } else if (operator == NE_EXPRESSION) {
        result =  !(left->type == LF_NULL_VALUE
                    && right->type == LF_NULL_VALUE);
    } else {
        char *op_str = lf_get_operator_string(operator);
        lf_runtime_error(line_number, NOT_NULL_OPERATOR_ERR,
                          STRING_MESSAGE_ARGUMENT, "operator", op_str,
                          MESSAGE_ARGUMENT_END);
    }
...
LF_String *
chain_string(LF_Interpreter *inter, LF_String *left, LF_String *right)
{
    int len;
    char *str;
    LF_String *ret;
    ...
    ret = lf_create_leaf_string(inter, str);
    lf_release_string(left);
    lf_release_string(right);

    return ret;
}

LF_Value
lf_eval_binary_expression(LF_Interpreter *inter, LocalEnvironment *env,
                           ...
                           Expression *left, Expression *right)
{
    ...
    if (left_val.type == LF_INT_VALUE
        ...
        eval_binary_double(inter, operator,
                           left_val.u.double_value, right_val.u.double_value,
                           &result, left->line_number);
    } else if (left_val.type == LF_BOOLEAN_VALUE
               && right_val.type == LF_BOOLEAN_VALUE) {
        result.type = LF_BOOLEAN_VALUE;
        result.u.boolean_value
            = eval_binary_boolean(inter, operator,
                                  ...
                                  left->line_number);
    } else if (left_val.type == LF_STRING_VALUE
               && operator == ADD_EXPRESSION) {
        char    buf[LINE_BUF_SIZE];
        LF_String *right_str;

        if (right_val.type == LF_INT_VALUE) {
            sprintf(buf, "%d", right_val.u.int_value);
            right_str = lf_create_leaf_string(inter, MEM_strdup(buf));
        } else if (right_val.type == LF_DOUBLE_VALUE) {
            sprintf(buf, "%f", right_val.u.double_value);
            right_str = lf_create_leaf_string(inter, MEM_strdup(buf));
        } else if (right_val.type == LF_BOOLEAN_VALUE) {
            if (right_val.u.boolean_value) {
                right_str = lf_create_leaf_string(inter,
                                                      MEM_strdup("true"));
            } else {
                right_str = lf_create_leaf_string(inter,
                                                      MEM_strdup("false"));
            }
        } else if (right_val.type == LF_STRING_VALUE) {
            right_str = right_val.u.string_value;
        } else if (right_val.type == LF_NATIVE_POINTER_VALUE) {
            sprintf(buf, "(%s:%p)",
                    right_val.u.native_pointer.info->name,
                    right_val.u.native_pointer.pointer);
            right_str = lf_create_leaf_string(inter, MEM_strdup(buf));
        } else if (right_val.type == LF_NULL_VALUE) {
            right_str = lf_create_leaf_string(inter, MEM_strdup("null"));
        } 
        result.type = LF_STRING_VALUE;
        result.u.string_value = chain_string(inter,
                                             left_val.u.string_value,
                                             right_str);
    } else if (left_val.type == LF_STRING_VALUE
               && right_val.type == LF_STRING_VALUE) {
        result.type = LF_BOOLEAN_VALUE;
        result.u.boolean_value
            = eval_compare_string(operator, &left_val, &right_val,
                                  left->line_number);
    } else if (left_val.type == LF_NULL_VALUE
               || right_val.type == LF_NULL_VALUE) {
        result.type = LF_BOOLEAN_VALUE;
        result.u.boolean_value
        ...
        char *op_str = lf_get_operator_string(operator);
        lf_runtime_error(left->line_number, BAD_OPERAND_TYPE_ERR,
                          STRING_MESSAGE_ARGUMENT, "operator", op_str,
                          MESSAGE_ARGUMENT_END);
    }
...
static LF_Value
eval_logical_and_or_expression(LF_Interpreter *inter,
                               ...
                               Expression *left, Expression *right)
{
    ...
    if (left_val.type != LF_BOOLEAN_VALUE) {
        lf_runtime_error(left->line_number, NOT_BOOLEAN_TYPE_ERR,
                          MESSAGE_ARGUMENT_END);
    }
    if (operator == LOGICAL_AND_EXPRESSION) {
        if (!left_val.u.boolean_value) {
            result.u.boolean_value = LF_FALSE;
            return result;
        }
    } else if (operator == LOGICAL_OR_EXPRESSION) {
        if (left_val.u.boolean_value) {
            result.u.boolean_value = LF_TRUE;
            return result;
        }
    } else {
    ...
    right_val = eval_expression(inter, env, right);
    if (right_val.type != LF_BOOLEAN_VALUE) {
        lf_runtime_error(right->line_number, NOT_BOOLEAN_TYPE_ERR,
                          MESSAGE_ARGUMENT_END);
    }
...
LF_Value
lf_eval_minus_expression(LF_Interpreter *inter, LocalEnvironment *env,
                          Expression *operand)
{
    LF_Value   operand_val;
    LF_Value   result;

    operand_val = eval_expression(inter, env, operand);
    if (operand_val.type == LF_INT_VALUE) {
        result.type = LF_INT_VALUE;
        result.u.int_value = -operand_val.u.int_value;
    } else if (operand_val.type == LF_DOUBLE_VALUE) {
        result.type = LF_DOUBLE_VALUE;
        result.u.double_value = -operand_val.u.double_value;
    } else {
        lf_runtime_error(operand->line_number, MINUS_OPERAND_TYPE_ERR,
                          MESSAGE_ARGUMENT_END);
    }
    return result;
}
...
static void
dispose_local_environment(LF_Interpreter *inter, LocalEnvironment *env)
{
    while (env->variable) {
        Variable        *temp;
        temp = env->variable;
        if (env->variable->value.type == LF_STRING_VALUE) {
            lf_release_string(env->variable->value.u.string_value);
        }
...
static LF_Value
call_native_function(LF_Interpreter *inter, LocalEnvironment *env,
                     Expression *expr, LF_NativeFunctionProc *proc)
{
    LF_Value   value;
    int         arg_count;
    ArgumentList        *arg_p;
    ...
    args = MEM_malloc(sizeof(LF_Value) * arg_count);
...
static LF_Value
call_leaf_function(LF_Interpreter *inter, LocalEnvironment *env,
                      Expression *expr, FunctionDefinition *func)
{
    ...
    for (arg_p = expr->u.function_call_expression.argument,
             param_p = func->u.leaf_f.parameter;
         arg_p;
         arg_p = arg_p->next, param_p = param_p->next) {
        LF_Value arg_val;

        if (param_p == NULL) {
            lf_runtime_error(expr->line_number, ARGUMENT_TOO_MANY_ERR,
                              MESSAGE_ARGUMENT_END);
        }
        arg_val = eval_expression(inter, env, arg_p->expression);
        lf_add_local_variable(local_env, param_p->name, &arg_val);
    }
    if (param_p) {
        lf_runtime_error(expr->line_number, ARGUMENT_TOO_FEW_ERR,
                          MESSAGE_ARGUMENT_END);
    }
    result = lf_execute_statement_list(inter, local_env,
                                        func->u.leaf_f.block
                                        ->statement_list);
...
static LF_Value
eval_function_call_expression(LF_Interpreter *inter, LocalEnvironment *env,
                              Expression *expr)
{
    LF_Value           value;
    ...
    func = lf_search_function(identifier);
    if (func == NULL) {
        lf_runtime_error(expr->line_number, FUNCTION_NOT_FOUND_ERR,
                          STRING_MESSAGE_ARGUMENT, "name", identifier,
                          MESSAGE_ARGUMENT_END);
    }
    switch (func->type) {
    case CROWBAR_FUNCTION_DEFINITION:
        value = call_leaf_function(inter, env, expr, func);
        break;
...
static LF_Value
eval_expression(LF_Interpreter *inter, LocalEnvironment *env,
                Expression *expr)
{
    LF_Value   v;
    switch (expr->type) {
    ...
    case LE_EXPRESSION:
        v = lf_eval_binary_expression(inter, env,
                                       expr->type,
    ...
    case MINUS_EXPRESSION:
        v = lf_eval_minus_expression(inter, env, expr->u.minus_expression);
        break;
...
LF_Value
lf_eval_expression(LF_Interpreter *inter, LocalEnvironment *env,
                    Expression *expr)
{
