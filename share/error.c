...
extern char *yytext;
extern MessageFormat lf_compile_error_message_format[];
...
self_check()
{
    if (strcmp(lf_compile_error_message_format[0].format, "dummy") != 0) {
        DBG_panic(("compile error message format error.\n"));
    }
    if (strcmp(lf_compile_error_message_format
    ...
    if (strcmp(lf_runtime_error_message_format[0].format, "dummy") != 0) {
        DBG_panic(("runtime error message format error.\n"));
    }
    if (strcmp(lf_runtime_error_message_format
...
void
lf_compile_error(CompileError id, ...)
{
    va_list     ap;
    VString     message;
    int         line_number;
    ...
    self_check();
    va_start(ap, id);
    line_number = lf_get_current_interpreter()->current_line_number;
    clear_v_string(&message);
    format_message(&lf_compile_error_message_format[id],
                   &message, ap);
...
void
lf_runtime_error(int line_number, RuntimeError id, ...)
{
    va_list     ap;
    VString     message;
    ...
    va_start(ap, id);
    clear_v_string(&message);
    format_message(&lf_runtime_error_message_format[id],
                   &message, ap);
    fprintf(stderr, "%3d:%s\n", line_number, message.string);
    va_end(ap);
